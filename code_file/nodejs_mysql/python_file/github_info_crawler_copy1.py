...
class get_info(object):
	def __init__(self, header, user_name):
		...
		self.repoIssue = {"open": list(), "closed": list()}
	...
	def repo_info_crawling(self):
		...
		for i, repo in enumerate(repo_names):
			...
			t4 = threading.Thread(target=repo_info.get_stats, args=(repo, self.repoStats,))
			t5 = threading.Thread(target=repo_info.get_branches, args=(repo, self.repoBranches,))
			t6 = threading.Thread(target=repo_info.get_issues, args=(repo, self.repoIssue,))
			t7 = threading.Thread(target=self.commits_crawler, args=(commit_data, committed_data, repo,))
			t8 = threading.Thread(target=repo_info.get_issue_comment, args=(repo, self.repoComment,))
			#------------------------------------------------------------------
			t1.start()
			t2.start()
			t4.start()
		...
		# q.put(etc_info)
		return repo_names_dict, total_repo_info, commit_ranks_key, self.repoLangs, self.repoTopics, self.userEvents, self.repoStats, self.repoBranches, self.repoIssue, self.repoComment
